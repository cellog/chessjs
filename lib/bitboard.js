...
Bitboard.prototype = {
 ...
 verticalFlip: function () {
  var newhigh = 0, newlow = 0, low = this.l, high = this.h
  // swap 1st/8th
  newhigh |= low << 24
  newlow  |= high >>> 24
  // swap 2nd/7th
 ...
 toArray: function(space) {
  space = space || ' '
  return this.debugString().replace(/0/g, space).replace(/1/g, this.piece).split('')
 },
 ...
 knightMoves: function(empty) {
  ...
  empty = empty || new Bitboard(0,0).not()
  return new Bitboard(res.l, res.h, this.piece).andB(empty)
 },
 kingMoves: function(empty) {
  var m = this,
 ...
 kingAttackTargets: function(notmine) {
  return this.kingMoves(notmine)
 },
 knightAttackTargets: function(notmine) {
  return this.knightMoves(notmine)
 },
 ...
 getConnectingRays: function(to, shouldbe, asArray, empty) {
  var tobit = to.to64Index(true),
      frombit = this.to64Index(true)
  if (frombit.length > 1) {
  ...
  if (tobit.length > 1) {
   var _this = this
   if (asArray) {
    return new ConnectingRays(_this, tobit, shouldbe, empty)
   }
   return tobit.reduce(function(t, b) {
    return t = t.orB(_this.getConnectingRays(Bitboard.from64Index(b), shouldbe, false, empty))
   }, new Bitboard(0,0))
  } else if (asArray) {
   return new ConnectingRays(this, tobit, shouldbe, empty)
  }
  tobit = tobit[0]
  frombit = frombit[0]
  var ret = new Bitboard(0,0), tomod = tobit % 8, frommod = frombit % 8,
      todiv = Math.floor(tobit / 8), fromdiv = Math.floor(fromdiv = frombit / 8)
  // vertical ray
  empty = empty || to.not()
  if ((shouldbe == 'rook' || shouldbe == 'queen') && tomod == frommod) {
   // generate vertical ray
   if (tobit > frombit) {
    ret =  this.fillUpOcclusion(empty).northOne()
   } else {
    ret = this.fillDownOcclusion(empty).southOne()
   }
  }
  if ((shouldbe == 'rook' || shouldbe == 'queen') && todiv == fromdiv) {
   // generate horizontal ray
   if (tobit > frombit) {
    ret = this.fillEastOcclusion(empty).eastOne()
   } else {
    ret = this.fillWestOcclusion(empty).westOne()
   }
  }
  if ((shouldbe == 'bishop' || shouldbe == 'queen') && Math.abs(todiv - fromdiv) == Math.abs(tomod - frommod)) {
   // generate diagonal ray
   if (todiv > fromdiv) {
    if (tomod > frommod) {
     ret = this.fillNortheastOcclusion(empty).northeastOne()
    } else {
     ret = this.fillNorthwestOcclusion(empty).northwestOne()
    }
   } else {
    if (tomod > frommod) {
     ret = this.fillSoutheastOcclusion(empty).southeastOne()
    } else {
     ret = this.fillSouthwestOcclusion(empty).southwestOne()
    }
   }
  }
  if (ret.andB(to).isNotEmpty()) {
   return ret
  }
  return new Bitboard(0,0)
 },
 to64Index: function(asArray) {
  if (!this.h && !this.l) {
 ...
 toVectors: function() {
  return Vector.fromBitboard(this)
 },
  ...
  */
 inCheck: function(empty, pawns, rooks, knights, bishops, queens, white, asVectors) {
  var byPawn, byRook, byBishop, byQueen,
      inCheck = (byPawn = this.pawnAttackTargets(white).andB(pawns))
         ...
         .orB(byKnight = this.knightMoves().andB(knights))
  if (inCheck.isEmpty()) {
   return false
  }
  var ret = {}
...
Bitboard.from64Index = function(index) {
 var newb = new Bitboard(1,0)
 index = 63-index
 if (index > 31) {
  newb = newb.BIGshl(index)
 } else if (index > 0) {
  newb = newb.shl(index)
 }
 return newb
...
function ConnectingRays(bitboard, tobit, shouldbe, empty)
{
 this._length = tobit.length
 this.squares = {}
 this.squarenames = []
 var a
 for (var i = 0; i < tobit.length; i++) {
  this.addRay(tobit[i], a = bitboard.getConnectingRays(Bitboard.from64Index(tobit[i]), shouldbe, false, empty))
  //console.log(Bitboard.from64Index(tobit[i]).toAlgebraic())
  //console.log(a.debugString("\n"))
 }
}

ConnectingRays.prototype = {
 addRay: function(square, ray) {
  var a
  this.squares[square] = this.squares[a = Bitboard.from64Index(square).toAlgebraic()] =
   ray
  this.squarenames.push(a)
 },
 debugString: function(square) {
  if (this.squares[square]) {
   return this.squares[square].debugString("\n")
  }
  throw new Error('Square not found "' + square + '"')
 },
 reduce: function(func, total) {
  total = total || this.squares[this.squarenames[0]]
  for (var i = 0; i < this._length; i++) {
   var square = this.squarenames[i]
   total = func(square, this.squares[square], total)
  }
  return total
 },
 get length() {
