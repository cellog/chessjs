...
Bitboard.prototype = {
 ...
 getConnectingRay: function(to, shouldbe) {
  ...
  if ((shouldbe == 'rook' || shouldbe == 'queen') && tomod == frommod) {
   ...
   if (tobit > frombit) {
    return from.fillUpOcclusion(to.not())
   } else {
    return from.fillDownOcclusion(to.not())
   }
  }
  if ((shouldbe == 'rook' || shouldbe == 'queen') && todiv == fromdiv) {
   // generate horizontal ray
   if (tobit > frombit) {
    return from.fillEastOcclusion(to.not())
   } else {
    return from.fillWestOcclusion(to.not())
   }
  }
  // diagonal
  ...
  if ((shouldbe == 'bishop' || shouldbe == 'queen') && todiv - fromdiv == tomod - frommod) {
   ...
   if (todiv > fromdiv) {
    if (tomod > frommod) {
     return from.fillNortheastOcclusion(to.not())
    } else {
     return from.fillNorthwestOcclusion(to.not())
    }
   } else {
    if (tomod > frommod) {
     return from.fillSoutheastOcclusion(to.not())
    } else {
     return from.fillSouthwestOcclusion(to.not())
    }
   }
  }
  return new Bitboard(0,0)
