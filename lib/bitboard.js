...
Bitboard.prototype = {
 ...
 getConnectingRays: function(to, shouldbe, asArray) {
  ...
  if (tobit.length > 1) {
   var _this = this
   if (asArray) {
    return new ConnectingRays(_this, tobit, shouldbe)
   }
   return tobit.reduce(function(t, b) {
    return t = t.orB(_this.getConnectingRays(Bitboard.from64Index(b), shouldbe))
   }, new Bitboard(0,0))
  } else if (asArray) {
   return new ConnectingRays(this, tobit, shouldbe)
  }
  tobit = tobit[0]
  frombit = frombit[0]
  var ret = to, tomod = tobit % 8, frommod = frombit % 8,
  ...
  if ((shouldbe == 'rook' || shouldbe == 'queen') && tomod == frommod) {
   ...
   if (tobit > frombit) {
    return this.fillUpOcclusion(to.not()).northOne()
   } else {
    return this.fillDownOcclusion(to.not()).southOne()
   }
  }
  if ((shouldbe == 'rook' || shouldbe == 'queen') && todiv == fromdiv) {
   // generate horizontal ray
   if (tobit > frombit) {
    return this.fillEastOcclusion(to.not()).eastOne()
   } else {
    return this.fillWestOcclusion(to.not()).westOne()
   }
  }
  if ((shouldbe == 'bishop' || shouldbe == 'queen') && Math.abs(todiv - fromdiv) == Math.abs(tomod - frommod)) {
   ...
   if (todiv > fromdiv) {
    if (tomod > frommod) {
     return this.fillNortheastOcclusion(to.not()).northeastOne()
    } else {
     return this.fillNorthwestOcclusion(to.not()).northwestOne()
    }
   } else {
    if (tomod > frommod) {
     return this.fillSoutheastOcclusion(to.not()).southeastOne()
    } else {
     return this.fillSouthwestOcclusion(to.not()).southwestOne()
    }
   }
  }
  return new Bitboard(0,0)
...
function ConnectingRays(bitboard, tobit, shouldbe)
{
 this._length = tobit.length
 this.squares = {}
 this.squarenames = []
 for (var i = 0; i < tobit.length; i++) {
  this.addRay(tobit[i], bitboard.getConnectingRays(Bitboard.from64Index(tobit[i]), shouldbe))
 }
}
