...
Bitboard.prototype = {
 ...
 algebraicToBitboard: function(coord) {
  if (!coord.match(/^[a-h][1-8]$/)) {
   throw new Error('Unknown coordinate: "' + coord + '"')
  }
  var index = (coord.toLowerCase().charCodeAt(0)-97+(coord.charCodeAt(1)-49)*8)
  if (index < 32) {
   return new Bitboard(1<<index, 0, this.piece)
  }
  return new Bitboard(0,1<<(index-32), this.piece)
 },
 bitboardToAlgebraic: function() {
  var mapit = function(i) {
   return ''.fromCharCode((i % 8) + 97) + ''.fromCharCode(Math.floor(i / 8) + 49)
  }
  return this.bitscan().map(mapit).reduce(function(t,c){t += ", " + c})
 },
 movePieceAlgebraic: function(from, to) {
  return this.movePiece(this.algebraicToBitboard(from), this.algebraicToBitboard(to))
 },
 movePieces: function(from, to) {
  if (from.h == to.h && from.l == to.l) {
   throw new Error('Cannot move from and to the same space (' + from.bitboardToAlgebraic() + ')')
  }
  if (this.andB(to).isNotEmpty()) {
   throw new Error('Cannot move to spaces that are already occupied (' + this.andB(to).bitboardToAlgebraic() + ')')
  }
 ...
 inCheck: function(empty, pawns, rooks, knights, bishops, queens, white) {
  ...
  var ret = {}
  byPawn.isEmpty() ? ret.pawn = byPawn : false
  byRook.isEmpty() ? ret.rook = byRook : false
  byBishop.isEmpty() ? ret.bishop = byBishop : false
  byQueen.isEmpty() ? ret.queen = byQueen : false
  return ret
 },
