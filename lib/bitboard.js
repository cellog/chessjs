...
Bitboard.prototype = {
 ...
 getConnectingRay: function(to, shouldbe) {
  ...
  // 0  1  2  3  4  5  6  7
  if ((shouldbe == 'bishop' || shouldbe == 'queen') && todiv - fromdiv == tomod - frommod) {
   // generate diagonal ray
   if (todiv > fromdiv) {
    if (tomod > frommod) {
     return this.fillNortheastOcclusion(to.not())
    } else {
     return this.fillNorthwestOcclusion(to.not())
    }
   } else {
    if (tomod > frommod) {
     return this.fillSoutheastOcclusion(to.not())
    } else {
     return this.fillSouthwestOcclusion(to.not())
    }
   }
  }
  return new Bitboard(0,0)
