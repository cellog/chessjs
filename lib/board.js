...
Bitboard.prototype.westOne = function() {
 return new Bitboard((this.low << 1) & notHFile, ((this.high << 1) | (this.low >>> 31)) & notHFile, this.piece)
}

Bitboard.prototype.northwestOne = function() {
 return new Bitboard(((this.low >>> 7) | (this.high << 25)) & notHFile, (this.high >> 7) & notHFile, this.piece)
}

Bitboard.prototype.southwestOne = function() {
 return new Bitboard((this.low << 9) & notHFile, ((this.high << 9) | (this.low >>> 23)) & notHFile, this.piece)
}

Bitboard.prototype.eastOne = function() {
 return new Bitboard(((this.low >>> 1) | (this.high << 31)) & notAFile, (this.high >> 1) & notAFile, this.piece)
}

Bitboard.prototype.northeastOne = function() {
 return new Bitboard(((this.low >>> 9) | (this.high << 23)) & notAFile, (this.high >> 9) & notAFile, this.piece)
}

Bitboard.prototype.southeastOne = function() {
 return new Bitboard((this.low << 7) & notAFile, ((this.high << 7) | (this.low >>> 25)) & notAFile, this.piece)
}

Bitboard.prototype.knightJumps = function() {
 var newlow = this.low, newhigh = this.high,
     shl = function(l,h,b) {
      return {low:l << b, high: (h << b) | (l >>> 32-b)}
     },
     shr = function(l,h,b) {
      return {low: (l >>> b) | (h << 32-b), high: h >> b}
     },
     and = function(t,a) {
      return {low:t.low & a, high: t.high & a}
     },
     or = function(t,v) {
      return {low: t.low | v.low, high: t.high | v.high}
     }
     l1 = and(shr(newlow, newhigh, 1),notAFile),
     l2 = and(shr(newlow, newhigh, 2),notABFile),
     r1 = and(shl(newlow, newhigh, 1),notHFile),
     r2 = and(shl(newlow, newhigh, 2),notGHFile),
     h1 = or(l1, r1),
     h2 = or(l2, r2),
     res = or(shl(h1.low, h1.high, 16),
              or(shr(h1.low, h1.high, 16),
                 or(shl(h2.low, h2.high, 8), shr(h2.low, h2.high, 8))))
 
 return new Bitboard(res.low, res.high, this.piece)
}
