...
board.prototype = {
 ...
 getCheckingLanes: function() {
  ...
  var empty = this.emptysquares,
      kingbishoplanes = king.bishopMoves(empty),
      kingrooklanes = king.rookMoves(empty),
      kingqueenlanes = kingbishoplanes.orB(empty)
  if (checks.bishop) {
   return checks.bishop.bishopMoves(empty).andB(kingbishoplanes)
  }
  if (checks.rook) {
   return checks.rook.rookMoves(empty).andB(kingrooklanes)
  }
  if (checks.queen) {
   // to do this properly, we need to have a method to check whether
   // rays intersect rather than the generic bishop moves/rook moves
   var diag, horiz
   diag = checks.queen.bishopMoves(empty).andB(kingbishoplanes)
   horiz = checks.queen.rookMoves(empty).andB(kingrooklanes)
  }
  return new Bitboard(0,0)
 },
 ...
 detectStartingSquare: function(bitboard, legalmoves, landing, p, startingSquare, captures) {
  switch (p) {
   case Pawn:
    if (captures) {
     var squares = bitboard.attackingPawns(legalmoves, whitesturn).toAlgebraic(true)
     if (squares.length == 1) {
      return squares[0]
     }
     return squares.reduce(function(i, t){if (i[0] == startingSquare || i[1] == startingSquare) return i;return t}, false)
    } else {
 ...
 parseAlgebraicNotation: function(ply, domove) {
  ...
  if (m) {
   ...
   if (!startingSquare || startingSquare.length != 2) {
    ...
    startingSquare = this.detectStartingSquare(bitboard, legalmoves, landing, p, startingSquare, !!captures)
   } else {
