...
board.prototype = {
 ...
 getCheckingLanes: function(asVector) {
  var whitesturn = this.turn == 'white',
      ...
      vectorking = whitesturn ? this.pieces.K[0] : this.pieces.k[0],
 ...
 detectStartingSquare: function(ply, bitboard, legalmoves, landing, p, startingSquare, captures) {
  ...
  switch (p) {
   case Pawn:
    if (captures) {
     var enemies = (this.turn == 'white' ) ? this.blackpieces : this.whitepieces
     if (enemies.andB(Bitboard.fromAlgebraic(landing)).isEmpty()) {
      throw new InvalidMoveError('Pawn capture at "' + landing + '" is impossible, no enemy piece on that square')
     }
     if (!startingSquare || !startingSquare.match(/^[a-h][1-8]?$/)) {
      throw new InvalidMoveError("Pawn capture move must be a file (a-h) or square")
     }
     ...
     if (startingSquare.length == 2) {
      var ret = squares[0]
      if (ret != startingSquare) {
       ret = squares[1]
      }
      if (-1 == pawnsquares.indexOf(ret)) {
       throw new InvalidMoveError('Pawn capture from "' + ret + '" to "' + landing + '" is impossible, no pawn on "' + ret + '"')
      }
      return ret
     } else {
     ...
     var lsq = chess.bitboard.fromAlgebraic(landing)
     if (lsq.andB(this.both).isNotEmpty()) {
      throw new InvalidMoveError('"' + ply + '" is not possible, there is a piece on that square')
     }
   ...
   case Knight:
    var possibles = legalmoves.andB(chess.bitboard.fromAlgebraic(landing)),tests = possibles
    if (possibles.isEmpty()) {
     throw new InvalidMoveError('Knights cannot move to "' + landing + '" from "' + bitboard.toAlgebraic() + '"')
    }
    possibles = bitboard.toAlgebraic(true)
    var newpossibles = []
    for (var i = 0; i < possibles.length; i++) {
     if (chess.bitboard.fromAlgebraic(possibles[i]).knightMoves().andB(tests).isEmpty()) {
      continue
     }
     if (startingSquare) {
      if (possibles[i][0] != startingSquare && possibles[i][1] != startingSquare) {
       continue
      }
     }
     newpossibles.push(possibles[i])
    }
    if (newpossibles.length == 1) {
     return newpossibles[0]
    } else {
     if (!startingSquare) {
      startingSquare = ply
     }
     throw new InvalidMoveError('Too many knights can move to "' + landing + '", need more disambiguation than "' + startingSquare + '"')
    }
   case Bishop:
    piecename = 'bishop'
   ...
   case Queen:
    ...
    break
   case King:
    var possibles = legalmoves.andB(chess.bitboard.fromAlgebraic(landing))
    if (possibles.isEmpty()) {
  ...
  var rays = land.getConnectingRays(bitboard, piecename, true, this.emptysquares)
  var possibles = rays.reduce(function(square, ray, total) {
   if (ray.isNotEmpty()) {
    if (startingSquare) {
     if (square[0] != startingSquare && square[1] != startingSquare) {
      return total
     }
    }
    total.push(square)
 ...
 parseAlgebraicNotation: function(ply, domove) {
  ...
  var m = ply.match(/^([PRNBQK])?([a-h]?[1-8]?)?(x)?([a-h][1-8])$/)
  if (!m) {
   throw new InvalidMoveError('"' + ply + '" is not valid algebraic notation')
  }
  var captures = m[3] || false
  var piece = m[1] ? m[1].toUpperCase() : 'P',p=map[piece],legalmoves
  var bitboard = this.bitboards[this.turn][p]
  ...
  if (bitboard.andB(Bitboard.fromAlgebraic(startingSquare)).isEmpty()) {
   throw new InvalidMoveError('No ' + namemap[piece] + ' exists on "' + startingSquare + '"')
  }
  return [startingSquare, landing]
 }
}
