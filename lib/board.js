 ...
 getLegalMoves: function(p, name, captures) {
  ...
  if (captures) {
   if (p == Pawn) {
    ...
    var kingtargets = bitboard.kingAttackTargets(this.turn == 'white' ? this.whitepieces.not() : this.blackpieces.not())
    return kingtargets.andB(this.bitboardAttacks(this.turn != 'white', kingtargets).not()).andB(enemy).andB(enemyking)
   } else {
