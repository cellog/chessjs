...
Bitboard.fromBinary = function(bin) {
 var high = bin.slice(0,32), low = bin.slice(-32)
 return new Bitboard(parseInt(high, 2), parseInt(low, 2))
}
...
Bitboard.prototype.northFill = function() {
 var high = this.high, low = this.low,
     lsh = function(numBits) {low = low << numBits; high = (high << numBits) | (low >>> (32 - numBits))},
     or  = function(l, h) {low = low | l, high = high | h}
 or(lsh(8))
 or(lsh(16))
 or(lsh(32))
...
Bitboard.prototype.southFill = function() {
 var high = this.high, low = this.low,
     rsh = function(numBits) {low = (low >>> numBits) | (high << (32 - numBits)); high = high >> numBits},
     or  = function(l, h) {low = low | l, high = high | h}
 or(rsh(8))
 or(rsh(16))
 or(rsh(32))
